...
$TaskName = 'GenshinDailyCoreRecorded'
...
function Show-LauncherMessage([string]$Message, [bool]$IsError = $false) {
    try {
        ...
        $null = $shell.Popup($Message, 15, '原神日常自动化 (前台录屏)', $icon)
    } catch {
        Write-Host $Message
    }
}
...
try {
    ...
    if ($task.Principal.RunLevel -ne 'Highest' -or
        ...
        throw ("计划任务 $TaskName 的权限或登录模式不正确，请重新安装计划任务。")
    }

    $recorderCheck = & (Join-Path $PSScriptRoot 'run_daily_recorded.ps1') -CheckOnly | ConvertFrom-Json
    if (-not $recorderCheck.Ready) {
        throw ('录屏环境未就绪：' + $recorderCheck.UnsupportedReason)
    }
    ...
    if ($CheckOnly) {
        [pscustomobject]@{
            ...
            Codec = [string]$recorderCheck.Codec
            Ffmpeg = [string]$recorderCheck.Ffmpeg
            AiRequired = $false
        } | ConvertTo-Json
        exit 0
    }
    ...
    $message = "原神前台录屏自动化已启动。`n运行编号：$($newRun.runId)`n录像将保存在 logs/runs/$($newRun.runId)/genshin-window.mkv`n无需保持此窗口打开。"
    ...
    $logPath = Join-Path $LauncherLogDir "start-recorded-$stamp.log"
