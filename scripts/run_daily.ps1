param([switch]$NoDelay, [int]$TimeoutMinutes = 0, [switch]$DryRun, [switch]$AllowRepeat, [ValidateSet('core','extras','configured')][string]$Profile = 'core', [ValidateSet('foreground','childSession')][string]$ExecutionMode)
...
try {
    ...
    if ($ExecutionMode) { $invokeArgs += @('--execution-mode', $ExecutionMode) }
